import type { BlockElement, Location, TextElement } from "../delta";
import { Editor, Path, Transforms } from "../delta";
...
export const setBlockNode = (
  ...
  if (!node) {
    ...
    const above = getAboveNode(editor, {
      at: location,
      match: (node) =>
        isBlock(editor, node) && (key ? existKey(node, key) : true),
    });
...
export const setWrapNodes = (
  ...
  Editor.withoutNormalizing(editor, () => {
    ...
    Transforms.wrapNodes(editor, itemConfig as BlockElement, {
      match: isText,
      at,
    });
  });
