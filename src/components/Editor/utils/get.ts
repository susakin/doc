import type { BaseNode, BlockElement, Location, Path } from "../delta";
import { Editor, ReactEditor } from "../delta";
...
export const getOmitAttributes = (
  ...
  keys.forEach(
    (item) => exclude.indexOf(item) === -1 && (result[item] = void 0)
  );
...
export const findNodePath = (
  ...
  try {
    ...
    while (queue.length) {
      ...
      if (!curNode || !curNode.node || !Array.isArray(curNode.node.children))
        continue;
 ...
 * @param at
 */
export const getClosestBlockPath = (
  editor: Editor,
  at: Location
): Path | null => {
 ...
 * @param depth > 0
 */
export const getNodeTupleByDepth = (
  editor: Editor,
  at: Location,
  depth: number
) => {
