...
import { Editor, Element, Path, Text } from "../delta";
...
export const existKey = (node: Node, key: string) =>
  isBaseElement(node) && !!node[key];
...
export const isBlock = (
  ...
  return Editor.isBlock(editor, node as any);
};
