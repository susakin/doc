...
import { Editor, Point, Range } from "../delta";
...
export function isWrappedEdgeNode(
  ...
  const wrapNode =
    options.wrapNode || getBlockNode(editor, { at, key: wrapKey });
  const itemNode =
    options.itemNode || getBlockNode(editor, { at, key: itemKey });
  if (wrapNode && itemNode && wrapNode.block.children.length) {
    ...
    if (mode === "last") {
      ...
      return (
        children[0] === itemNode.block ||
        children[children.length - 1] === itemNode.block
      );
    }
  }
  return false;
